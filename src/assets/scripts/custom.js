...
$(window).load(function() { 
	...
	$("#pollResBasic").css('display', 'none'); 
});
$(document).ready(function () {
	/* yes - no bar chart */
	$('#bar').each(function() {
		var bar = $(this);
		var size = bar.data("size"); 
		var pheight = bar.height();
		var fheight = ( (size / 100 ) * pheight );
		var poll = bar.data("poll");
	...
	$('#fbar').each(function() {
		...
		var pheight = bar.height();
		var fheight = ( (size / 100 ) * pheight ); 
		var poll = bar.data("poll").toLocaleString();
...
$( document ).ready(function() { 
	...
	$("input[type='submit']").click(function(){
        var $fileUpload = $("input.pollImage");
	...
	$('.close-wrap').on("click", function () {
		$(this).parents('#search').fadeOut(100);
	});
	
	//open add-new
	$("#add-new").click(function () { 	
		$("#add-new-wrap").toggle(100);
	});
	//close add-new with close button
	$('.close-new').on("click", function () {
		$(this).parents('#add-new-wrap').fadeOut(100);
	});
	//open registerWrap
	$(".triggerRegister").click(function () { 	
		$("#registerWrap").toggle(100);
	});
